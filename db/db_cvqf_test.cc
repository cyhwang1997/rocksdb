...
class DBCVQFTest : public DBTestBase {
 public:
  DBCVQFTest() : DBTestBase("/db_cvqf_test") {}
};

class DBCVQFTestWithParam
    : public DBTestBase,
 ...
 public:
  DBCVQFTestWithParam() : DBTestBase("/db_cvqf_tests") {}
...
class DBCVQFTestDefFormatVersion : public DBCVQFTestWithParam {};
...
TEST_P(DBCVQFTestDefFormatVersion, KeyMayExist) {
  do {
    ReadOptions ropts;
    std::string value;
...
INSTANTIATE_TEST_CASE_P(
    FormatDef, DBCVQFTestDefFormatVersion,
...
INSTANTIATE_TEST_CASE_P(
    FormatLatest, DBCVQFTestWithParam,
...
class WrappedCVQF : public FilterPolicy {
 public:
  explicit WrappedCVQF(int bits_per_key)
      : filter_(NewBloomFilterPolicy(bits_per_key)), counter_(0) {}

  ~WrappedCVQF() override { delete filter_; }
...
TEST_F(DBCVQFTest, CVQFWrapper) {
  ...
  BlockBasedTableOptions table_options;
  WrappedCVQF* policy = new WrappedCVQF(10);
...
TEST_F(DBCVQFTest, PrefixExtractorBlockFilter) {
  BlockBasedTableOptions bbto;
  // Block Filter Block
...
class CVQFStatsTestWithParam
 ...
 public:
  ...
  ~CVQFStatsTestWithParam() override {
    get_perf_context()->Reset();
    Destroy(options_);
  }
...
TEST_P(CVQFStatsTestWithParam, CVQFStatsTest) {
...
TEST_P(CVQFStatsTestWithParam, CVQFStatsTestWithIter) {
  std::string key1("AAAA");
  std::string key2("RXDB");  // not in DB
  std::string key3("ZBRA");
  std::string value1("Value1");
...
INSTANTIATE_TEST_CASE_P(CVQFStatsTestWithParam, CVQFStatsTestWithParam,
...
namespace {
void PrefixScanInit(DBCVQFTest* dbtest) {
  char buf[100];
  std::string keystr;
...
TEST_F(DBCVQFTest, OptimizeFiltersForHits) {
  Options options = CurrentOptions();
...
// behaves as expected
TEST_F(DBCVQFTest, DynamicCVQFOptions) {
  int iteration = 0;
  for (bool use_block_based_builder : {true, false}) {
    Options options;
    options.create_if_missing = true;
