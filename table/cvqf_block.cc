...
CVQFBlockBuilder::CVQFBlockBuilder(
...
void CVQFBlockBuilder::Add(const Slice& key) {
...
inline void CVQFBlockBuilder::AddKey(const Slice& key) {
  filter_bits_builder_->AddKey(key);
  num_added_++;
}

// Add prefix to filter if needed
inline void CVQFBlockBuilder::AddPrefix(const Slice& key) {
...
void CVQFBlockBuilder::Reset() {
  last_whole_key_recorded_ = false;
  last_prefix_recorded_ = false;
}

Slice CVQFBlockBuilder::Finish(const BlockHandle& /*tmp*/,
                                     Status* status) {
  Reset();
...
CVQFBlockReader::CVQFBlockReader(
...
CVQFBlockReader::CVQFBlockReader(
    ...
    Statistics* stats)
    : CVQFBlockReader(prefix_extractor, _whole_key_filtering,
                            contents.data, filter_bits_reader, stats) {
  block_contents_ = std::move(contents);
}

bool CVQFBlockReader::KeyMayMatch(
...
bool CVQFBlockReader::PrefixMayMatch(
...
bool CVQFBlockReader::MayMatch(const Slice& entry) {
  if (contents_.size() != 0)  {
...
size_t CVQFBlockReader::ApproximateMemoryUsage() const {
...
bool CVQFBlockReader::RangeMayExist(const Slice* iterate_upper_bound,
...
bool CVQFBlockReader::IsFilterCompatible(
