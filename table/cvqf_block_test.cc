...
class PluginCVQFBlockTest : public testing::Test {
 public:
...
TEST_F(PluginCVQFBlockTest, PluginSingleChunk) {
  CVQFBlockBuilder builder(
      nullptr, true, table_options_.filter_policy->GetFilterBitsBuilder());
  builder.Add("foo");
  builder.Add("bar");
...
class CVQFBlockTest : public testing::Test {
 public:
  BlockBasedTableOptions table_options_;
...
TEST_F(CVQFBlockTest, EmptyBuilder) {
  CVQFBlockBuilder builder(
      nullptr, true, table_options_.filter_policy->GetFilterBitsBuilder());
  Slice block = builder.Finish();
  ASSERT_EQ("", EscapeString(block));
...
TEST_F(CVQFBlockTest, DuplicateEntries) {
  {  // empty prefixes
...
TEST_F(CVQFBlockTest, SingleChunk) {
  CVQFBlockBuilder builder(
      nullptr, true, table_options_.filter_policy->GetFilterBitsBuilder());
  ASSERT_EQ(0, builder.NumAdded());
  builder.Add("foo");
